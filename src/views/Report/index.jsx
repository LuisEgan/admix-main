...
import { getReportData } from "actions/app";
...
Date.prototype.takeDays = function(days) {
  var dat = new Date(this.valueOf());
  dat.setDate(dat.getDate() - days);
  return dat;
};
...
export default class Placement extends Component {
  ...
  constructor(props) {
    ...
    this.isSceneLoaded = this.isSceneLoaded.bind(this);
  ...
  componentDidMount() {
    ...
    this.setState({ userApps, selectedApps });
  }
  ...
  previousPeriods(max) {
    ...
    for (let i = 0; i < max; i++) {
      previousPeriods.push(
        this.filteredData(
          from.takeDays(daysInterval * intervalCounter),
          to.takeDays(daysInterval * intervalCounter)
        )
      );
      intervalCounter++;
    }
  ...
  renderAppsDropdown() {
    ...
    const allAppsDropwdown = _dropdownStruct("Select an app", dropdown);
  ...
  render() {
    const { show, parseDate, getDates, filteredData, previousPeriods } = this;
    const {
      from,
      to,
      doLoadScene,
    ...
    const { dispatch, isLoad_webgl, reportData } = this.props;
    ...
    return (
      <div id="report">
        ...
        <div className={`container panel menu-panel`}>
          <div id="app-selection" className="container">
            <h6 className="st">Reporting</h6>
          ...
          <div className="menu-panel-vertical-cover" style={performanceShow} />
        </div>
        ...
        <ToggleDisplay show={show("pe")}>
          <Performance
            ...
            TrackballControlsEnableWheel={this.TrackballControlsEnableWheel}
            TrackballControlsDisableWheel={this.TrackballControlsDisableWheel}
            noPointerLockControlsRotation={this.noPointerLockControlsRotation}
            yesPointerLockControlsRotation={this.yesPointerLockControlsRotation}
